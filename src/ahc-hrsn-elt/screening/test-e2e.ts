...
async function ingressWorkflow(
  ...
  if (workflow?.duckdb.stdErrsEncountered) {
    ...
    console.error(
      `❌ ${c.brightRed(
        "DuckDB orchestration SQL syntax/parsing errors encountered (ingestion state is indeterminate)."
      )}`
    );
  } else {
    console.info(
  ...
  if (resourceDbSupplier) {
    ...
    console.info(
      `📦 ${c.green(
        resourceDbSupplier()
      )} has the aggregated content and \`orch_session_*\` validation tables.`
    );
  }
  ...
  console.info(
    `🦆 ${c.yellow(
      e2eTestPaths.inProcess.duckDbFsPathSupplier()
    )} has the raw ingested content and \`orch_session_*\` validation tables.`
  );
}
...
function e2eTestWorkflowPaths(rootPath: string): mod.OrchEngineWorkflowPaths {
  ...
  const ingressTx: mod.OrchEngineWorkflowPaths["ingressTx"] = {
    ...oeStorablePath("support/assurance/ahc-hrsn-elt/screening/ingress-tx"),
  };
